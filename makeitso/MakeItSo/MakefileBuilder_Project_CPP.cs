...
namespace MakeItSo
{
  ...
  class MakefileBuilder_Project_CPP
  {
    ...
    private void createConfigurationTarget(ProjectConfigurationInfo_CPP configurationInfo)
    {
      ...
      if (!string.IsNullOrEmpty(m_projectInfo.PrecompiledHeader))
      {
        foreach (string filename in m_projectInfo.getFiles())
        {
          if (string.Compare(filename, m_projectInfo.PrecompiledHeaderFileNameMatch, true) == 0)
          {
            string path = String.Format("{0}/{1}", intermediateFolder, filename);
            string objectPath = Path.ChangeExtension(path, ".h.gch");
    ...
    private void createFileTargets(ProjectConfigurationInfo_CPP configurationInfo)
    {
      ...
      //precompiled header, need to merge with below as it is a rough cop
      if (!string.IsNullOrEmpty(m_projectInfo.PrecompiledHeader))
      {
        foreach (string filename in m_projectInfo.getFiles())
        {
          if (string.Compare(filename, m_projectInfo.PrecompiledHeaderFileNameMatch, true) == 0)
          {
            ...
            m_file.WriteLine("# Compiles file {0} as the precompiled header {0}.gch for the {1} configuration...", m_projectInfo.PrecompiledHeader, configurationInfo.Name);
            m_file.WriteLine("-include {0}", dependenciesPath);
            m_file.WriteLine("{0}: {1}", objectPath, m_projectInfo.PrecompiledHeader);

            m_file.WriteLine("\t{0} {1} {2} -c {3} {4} -o {5}", compiler, preprocessorDefinitions, compilerFlags, m_projectInfo.PrecompiledHeader, includePath, m_projectInfo.PrecompiledHeaderBuiltName);
            m_file.WriteLine("");
